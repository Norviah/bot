...
export default class extends Listener {
   ...
   */
  public constructor() {
    super({ event: 'messageReactionRemove', emitter: 'client' });
  }
  ...
  public async exec(reaction: MessageReaction, user: User): Promise<void> {
    ...
    if (member) {
      member.roles
        .remove(role)
        .catch((error: Error) => this.client.log(guild, { description: `I tried to remove the role ${role} from ${member}, but an error occurred: ${error.message}.`, event: null }));
    }
  }
}
