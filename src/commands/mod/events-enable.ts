...
const options: CommandOptions = {
   ...
   */
  description: 'As certain events can be logged into a specific channel, use this command to enable desired events.',
  ...
  args: [
    {
      ...
      type: [...keys, 'all'],
      match: 'separate',
      prompt: { start: 'which event(s) would you like to enable?', retry: `please provide a valid event, one of \`${join(keys, 'or')}\`` },
    },
  ],
...
/**
 * Allows moderators to enable specific events for the client to log.
 */
export default class extends Command {
   ...
   * @param  args.events The events to enable for this guild.
   */
  public async exec(message: Message, args: { events: (LoggedEvents | 'all')[] }): Promise<any> {
    ...
    for (const event of args.events) {
      if (event !== 'all') {
        this.client.database.put(message.guild!, 'events', event, true);
      }
    }

    await this.client.confirm(message, `Enabled the events: ${this.client.join(args.events.map((event: string) => `**${event}**`))}.`);
  }
}
